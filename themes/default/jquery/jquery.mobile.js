	...
	setup: function() {
		...
		$this
			.bind( touchStartEvent, function( event ) {
				...
				// Get the correct event values for pageX and pageY **TZAdvantage**
				// (in iOS event.pageX and event.pageY are always 0 )
				// using the same code as in $.event.special.swipe!!
				...
				var moved = false,
					touching = true,
					// use data instead of event. **TZAdvantage**
					origPos = [ data.pageX, data.pageY ],
					originalType,
					timer;

				function moveHandler() {
					// use data instead of event. **TZAdvantage**
					if ((Math.abs(origPos[0] - data.pageX) > 10) ||
						(Math.abs(origPos[1] - data.pageY) > 10)) {
						moved = true;
					}
				}
				...
				timer = setTimeout(function() {
					if ( touching && !moved ) {
						...
						event.type = "taphold";
						// overwrite pageX,pageY with stored coordinates **TZAdvantage**
						event.pageX = origPos[0] + 10;
